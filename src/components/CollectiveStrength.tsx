        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {GROUP_COMPANIES.map((company) => (
            <div
              ...
              className="p-6 bg-white border border-[#8B6914]/20 rounded-sm hover:border-[#8B6914] hover:shadow-md transition-all duration-300 cursor-pointer group flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-serif font-bold text-[#8B6914]">
                    {company.number}
                  </span>
                  <span className="text-[10px] uppercase tracking-wider text-[#4A4843] font-medium px-2 py-0.5 rounded bg-gray-100">
                    {company.industry.split('&')[0]}
                  </span>
                </div>

                <h3 className="text-lg font-serif font-bold text-[#2C2D31] group-hover:text-[#8B6914] transition-colors mb-2">
                  {company.name}
                </h3>

                <p className="text-xs text-[#4A4843] font-normal leading-relaxed line-clamp-3">
                  {company.description}
                </p>
              </div>

              <div className="pt-4 mt-4 border-t border-gray-100 flex items-center justify-between text-[11px] font-semibold text-[#8B6914]">
